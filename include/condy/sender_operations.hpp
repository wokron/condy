...
template <typename Func, typename... Args>
auto make_op_sender128(Func &&func, Args &&...args) {
    auto prep_func = [func = std::forward<Func>(func),
                      ... args = std::forward<Args>(args)](Ring *ring) {
...
auto make_multishot_op_sender(MultiShotFunc &&multishot_func, Func &&func,
                              Args &&...args) {
    auto prep_func = [func = std::forward<Func>(func),
                      ... args = std::forward<Args>(args)](Ring *ring) {
        auto *sqe = ring->get_sqe();
    ...
    return build_multishot_op_sender<SimpleCQEHandler>(
        std::move(prep_func), std::forward<MultiShotFunc>(multishot_func));
}

template <BufferRingLike Br, typename Func, typename... Args>
auto make_select_buffer_op_sender(Br *buffers, Func &&func, Args &&...args) {
    ...
    return build_op_sender<SelectBufferCQEHandler<Br>>(std::move(prep_func),
                                                       buffers);
}

template <typename MultiShotFunc, BufferRingLike Br, typename Func,
          typename... Args>
auto make_multishot_select_buffer_op_sender(MultiShotFunc &&multishot_func,
    ...
    return build_multishot_op_sender<SelectBufferCQEHandler<Br>>(
        std::move(prep_func), std::forward<MultiShotFunc>(multishot_func),
        buffers);
}
...
template <BufferRingLike Br, typename Func, typename... Args>
auto make_bundle_select_buffer_op_sender(Br *buffers, Func &&func,
                                         Args &&...args) {
    auto prep_func = [bgid = buffers->bgid(), func = std::forward<Func>(func),
                      ... args = std::forward<Args>(args)](Ring *ring) {
        auto *sqe = ring->get_sqe();
...
template <typename MultiShotFunc, BufferRingLike Br, typename Func,
          typename... Args>
auto make_multishot_bundle_select_buffer_op_sender(
...
auto make_zero_copy_op_sender(FreeFunc &&free_func, Func &&func,
                              Args &&...args) {
    auto prep_func = [func = std::forward<Func>(func),
                      ... args = std::forward<Args>(args)](Ring *ring) {
        auto *sqe = ring->get_sqe();
    ...
    return build_zero_copy_op_sender<SimpleCQEHandler>(
